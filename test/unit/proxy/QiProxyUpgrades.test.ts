...
!developmentChains.includes(network.name)
    ...
    : describe("Upgrading tests", function () {
          let qi: Qi,
              qiProxy: TransparentUpgradeableProxy,
              qiBackground: QiBackground,
              qiBackgroundProxy: TransparentUpgradeableProxy,
              qiProxyAdmin: ProxyAdmin,
              qiBackgroundProxyAdmin: ProxyAdmin
          beforeEach(async () => {
          ...
          it("can deploy and upgrade qiBackground", async function () {
              ...
              // Tests if the proxy upgrade doesn't fail
              const upgradeTx = await qiBackgroundProxyAdmin.upgrade(
                  qiBackgroundProxy.address,
                  newQiBackground.address
              )
              await upgradeTx.wait(1)
